...
def write_proceeding_info():
    bibtex_str = '@Proceedings{CLeaR-2023,\n'
    bibtex_str += '\tbooktitle = {Proceedings of The 2nd Conference on Causal Learning and Reasoning},\n'
    bibtex_str += '\tname = {Conference on Causal Learning and Reasoning},\n'
    bibtex_str += '\tshortname = {CLeaR},\n'
    bibtex_str += '\teditor = {van der Schaar, Mihaela and Zhang, Cheng and Janzing, Dominik},\n'
    bibtex_str += '\tvolume = {213},\n'
    bibtex_str += '\tyear = {2023},\n'
    bibtex_str += '\tstart = {2022-04-11},\n'
    bibtex_str += '\tend = {2022-04-14},\n'
    bibtex_str += '\tconference_url = {https://www.cclear.cc/},\n'
    bibtex_str += '\taddress = {Tubingen, Germany},\n'
    bibtex_str += '\tconference_number={2},\n'
    bibtex_str += '}\n\n'
    return bibtex_str
...
def create_identifiers(all_metadata):
    ...
    for i, metadata in zip(range(len(all_metadata)), all_metadata):
        identifier = (metadata['submission_content']['authors'][0].split(' ')[-1]).lower() + YEAR
...
def rename_files(indir, outdir, metadata, bibtex_str):
    ...
    if False: # not supporting supplementary material 
       # 'supplementary_material' in metadata['submission_content'].keys() and metadata['submission_content']['supplementary_material'] != "":
